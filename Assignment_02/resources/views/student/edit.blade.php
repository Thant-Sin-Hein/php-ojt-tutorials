...
@section('content')
     ...
     <div class="panel-body border border-1 w-75 mt-5 me-0 ms-auto me-auto">
        <div class=" pt-3 pb-3 bg-light w-100 border border-1"><h4 class=" ms-4">Student Update</h4></div>
         <!-- New Task Form -->
         <form action="{{ url('student-edit/'.$student->id) }}" method="POST" class="form-horizontal mt-3 mb-3 ms-4 me-4 ">
             {{ csrf_field() }}
             @method('PUT')
             <!-- Task Name -->
             ...
             <div class="form-group mt-2">
                 ...
                 <div class="col-sm-12">
                     <textarea name="address" cols="10" rows="2" class="w-100 form-control">{{$student->address}}</textarea>
                 </div>
             </div>
             @error('address')
                <span class="text-danger">{{ $message }}</span>
